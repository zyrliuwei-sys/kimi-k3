...
export const docCollectionDocument = table(
  'doc_collection_document',
  {
    ...
    // Full extracted text (up to 1M tokens ≈ 4MB chars). Kept inline so
    // buildMessages() can pass it directly into the K3 prompt without an
    // extra read round-trip. If a doc is larger than 1M chars the parser
    // truncates + sets parseStatus='truncated'.
    contentText: text('content_text'),
    // JSON: { pageMap: [{page: 1, charStart, charEnd}], title?, author? }
...
export const docCollectionMessage = table(
  'doc_collection_message',
  {
    ...
    role: text('role').notNull(),
    content: text('content').notNull(),
    // JSON array: [{docId, filename, page, quote}]
...
// ─── PPT Generator ──────────────────────────────────────────────────────────
// /ppt feature: one-click PPT generation. A `ppt_task` row tracks each
// generation job from "queued" through "outlining" → "writing" → "rendering"
// → "done" (or "failed"). The K3 outline + per-slide content are stored in
// `outlineJson` so the user can revisit and regenerate later.
...
export const pptTask = table(
  'ppt_task',
  {
    ...
    // Source: "doc_collection:<id>" | "text:<base64>" | "empty"
    sourceType: text('source_type').notNull(),
    sourceRef: text('source_ref').notNull().default(''),
    prompt: text('prompt').notNull().default(''),
    status: text('status').notNull().default('queued'),
    // 0..100 — drives the front-end progress bar
    progress: integer('progress').notNull().default(0),
    // JSON: outline returned by K3
    outlineJson: text('outline_json'),
    // JSON: per-slide content blocks
    slidesJson: text('slides_json'),
    // Where the rendered .pptx lives (R2 / local fallback)
